...
int CPcbProcess2::Ready_process(int iStep)
{
	...
	case 10160:
		...
		{
			//ÀÚµ¿¿îÀüÁß ÇÏ³ª¾¿¸¸ ÄÑÁ®¾ßµÅ¼­ ¸ðµÎ Off
			Sleep(50);
...
int	CPcbProcess2::Complete_FinalInsp(int iStep)
{
	...
	case 122565:
		...
		{
			sLog.Format("PCBºÎ ¸ðÅÍ ÀÌ¹° °Ë»ç À§Ä¡ ÀÌµ¿ ½ÇÆÐ[%d]", iStep);
			putListLog(sLog);
			iRtnFunction = -122565;
			break;
		}
	...
	case 122575:
		...
		{
			sLog.Format("PCB Z Axis ÀÌ¹°°Ë»ç Pos Move Complete[%d]", iStep);
			putListLog(sLog);
			Task.PCBTaskTime = myTimer(true);
			iRtnFunction = 122580;
			break;
		...
		{
			sLog.Format("PCB Z Axis ÀÌ¹°°Ë»ç Pos Move Fail[%d]", iStep);
			errMsg2(Task.AutoFlag, sLog);
			iRtnFunction = -122575;
			break;
		}
		...
		break;
	case 122580:
		//Dome ÀÌ¹° ±¤¿ø¿¡¼­ °Ë»çÇÏ´Â Ç×¸ñ
		//Lens Shading - 7Àå ÀÌ¹ÌÁö 4 + 3
		//-----------------------------------------------------------------
		//Blemish - 1Àå ÀÌ¹ÌÁö A Mode
		//RI - 1Àå ÀÌ¹ÌÁö A Mode
		//Black Mask Edge Test - 1Àå ÀÌ¹ÌÁö A Mode
		//Illumination Center - 1Àå ÀÌ¹ÌÁö A Mode
		//SNR - 1Àå ÀÌ¹ÌÁö A Mode
		//R/G , B/G - 1Àå ÀÌ¹ÌÁö A Mode
		//-----------------------------------------------------------------
		//Defect - 9Àå ÀÌ¹ÌÁö
		//Dark Noise - ÃÑ 60Àå 30(H) + 30(I)
		if (LensShadingIndex == 0)
		{
			beolRtn = MIU.CheetahModeChange(A_MODEL);		//¸ðµå º¯°æ
		}
		if (LensShadingIndex == 4)
		{
			beolRtn = MIU.CheetahModeChange(B_MODEL);		//¸ðµå º¯°æ
		}
		if (beolRtn == false)
		{
	...
	case 122585:
		...
		if (LensShadingIndex == 0){
			...
			sLog.Format("Green ±¤¿ø On[%d]", iStep);
			putListLog(sLog);
	...
	case 122590:
		...
		{
			LightRetry++;
			//r,b,g °ªÀÌ ¸ÂÃçÁ³´ÂÁö È®ÀÎ ÇÊ¿ä.
			TCHAR* pszCol[] = { _T("R Color"), _T("G Color"), _T("B Color") };
			if (MIU.Get_Cheetah_Light_Value(Task.chkTargetRgb, Task.chkTargetChannel))
			{

				sLog.Format("%s Color: %d ¼³Á¤ ¿Ï·á[%d]", pszCol[Task.chkTargetChannel], Task.chkTargetRgb, iStep);
				putListLog(sLog);
				iRtnFunction = 122600;
				break;
			}
			iRtnFunction = 122585;		//¹Ýº¹ Á¶¸í ÄÁÆ®·Ñ·¯ Á¶Á¤
			break;
		}
		else
		{
			//½ÇÆÐ
			sLog.Format("¹à±â Á¶Á¤ Fail[%d]", iStep);
			errMsg2(Task.AutoFlag, sLog);
			iRtnFunction = -122590;
			break;
		}
	...
	case 122600:
		...
		else
		{
			...
			putListLog(sLog);
			iRtnFunction = 122580;		//¸ðµå ÀüÈ¯ ÈÄ Ä¸Ã³
		}
	...
	case 122704:
		...
		if (LightRetry < MaxLightRetry)		//50
		{
			...
			iRtnFunction = 122703;		//¹Ýº¹ Á¶¸í ÄÁÆ®·Ñ·¯ Á¶Á¤
	...
	case 122705:
		...
		putListLog(sLog);
		iRtnFunction = 122706;		//¸ðµå ÀüÈ¯ ÈÄ Ä¸Ã³
	...
	case 123001:
		...
		{
			LightRetry = 0;
			beolRtn = MIU.CheetahModeChange(F_MODEL);		//¸ðµå º¯°æ
			sLog.Format("F Mode Change Ok[%d]", iStep);
		}
		if (DefectIndex == 1)
		{
			LightRetry = 0;
			beolRtn = MIU.CheetahModeChange(G_MODEL);		//¸ðµå º¯°æ
	...
	case 123004:
		if (LightRetry < MaxLightRetry)		//50
		{
			...
			iRtnFunction = 123002;		//¹Ýº¹ Á¶¸í ÄÁÆ®·Ñ·¯ Á¶Á¤
			break;
		}
		else
		{
	...
	case 123005:
		...
		iRtnFunction = 123006;		//Defect °Ë»ç ¿Ï·á
		break;

	case 123006:
		//
		//
		//Bright Defect °Ë»ç 2¹ø
		//
	...
	case 123300:
		...
		{
			sLog.Format("PCB Z Á¤ÁöÈ®ÀÎ ½Ã°£ ÃÊ°ú[%d]", iStep);
			errMsg2(Task.AutoFlag, sLog);
			iRtnFunction = -123300;
			break;
		}
	...
	case 123400:
		DefectIndex = 0;
		iRtnFunction = 123401;
		//Dark Noise °Ë»ç ½ÃÀÛ - Dark ¿¡¼­´Â Á¶¸í Á¶Àý¾øÀÌ Ä«¸Þ¶ó ¸ðµå¸¸ º¯°æ
		break;
	...
	case 123401:
		...
		if (DefectIndex == 0)
		{
			beolRtn = MIU.CheetahModeChange(E_MODEL);		//1Àå
			sLog.Format("E Mode Change Ok[%d]", iStep);
		}
		if (DefectIndex == 1)
		{
			beolRtn = MIU.CheetahModeChange(F_MODEL);		//1Àå
			sLog.Format("F Mode Change Ok[%d]", iStep);
		}
		if (DefectIndex == 2)
		{
			beolRtn = MIU.CheetahModeChange(G_MODEL);		//1Àå
			sLog.Format("G Mode Change Ok[%d]", iStep);
		}
	...
	case 123406:
		...
		if (DefectIndex == 0)
		{
			beolRtn = MIU.CheetahModeChange(H_MODEL);		//30Àå
			sLog.Format("H Mode Change Ok[%d]", iStep);
		}
		if (DefectIndex == 1)
		{
			beolRtn = MIU.CheetahModeChange(I_MODEL);		//30Àå
	...
	case 123408:
		...
		if (DarkGetCount < 60)
		{
			...
			{
				DefectIndex = 1;
				iRtnFunction = 123406;//¸ðµå º¯°æ
				break;
			}
			iRtnFunction = 123409;
			break;
		}
		//Dark Image 60Àå Ä¸Ã³ ¿Ï·á
